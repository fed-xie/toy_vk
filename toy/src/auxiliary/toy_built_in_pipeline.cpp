...
{
	VkResult vk_err;
	constexpr uint32_t cmd_count = 1;
	for (uint32_t i = 0; i < TOY_CONCURRENT_FRAME_MAX; ++i) {
		...
		vk_err = vkAllocateCommandBuffers(vk_driver->device.handle, &cmd_buffer_ai, pipeline->render_pass.cmds[i].draw_cmds);
		if (toy_unlikely(VK_SUCCESS != vk_err)) {
			...
			for (uint32_t j = i; j > 0; --j) {
				vkFreeCommandBuffers(
					...
					cmd_count,
					pipeline->render_pass.cmds[j].draw_cmds);
			}
			return;
		}
	}
...
FAIL_PASS_MAIN_CAMERA_CMD:
	toy_destroy_built_in_vulkan_render_pass_main_camera(
...
{
	VkResult vk_err;
	toy_built_in_vulkan_frame_resource_t* frame_res = &pipeline->frame_res[vk_driver->swapchain.current_frame];
	VkCommandBuffer draw_cmd = pipeline->render_pass.cmds[vk_driver->swapchain.current_frame].draw_cmds[0];

	reset_frame_resource(vk_driver, frame_res, error);
	if (toy_is_failed(*error))
		goto FAIL_RESET_FRAME_RESOURCE;

	toy_prepare_render_pass_main_camera(frame_res, scene, &pipeline->render_pass.main_camera);

	toy_unmap_vulkan_buffer_memory(
		vk_driver->device.handle, &frame_res->uniform_stack.buffer, error);
	TOY_ASSERT(toy_is_ok(*error));
	...
	toy_draw_render_pass_main_camera(
		frame_res,
		scene,
		vk_driver,
